...
그다음 Codex Desktop의 plugin marketplace UI에서 `codex-fable5@fablecodex`를 설치하고 활성화합니다. `codex-cli 0.128.0`에서는 plugin enablement가 app-server/Desktop을 통해 이뤄지며, `codex plugin add`는 공개 CLI subcommand가 아닙니다.

Codex를 다시 시작하거나 plugin list를 reload한 뒤, 프롬프트에서 skill을 호출합니다.
...
여러 dependent story가 있거나, 검증 위험이 높거나, review finding이 최종 완료를 막아야 할 때 ledger를 사용하세요. 짧은 작업에서는 local JSON ledger가 작업보다 무거울 수 있으니 일반 Codex plan을 쓰면 충분합니다.
...
| `codex-fable5 doctor` | 설치된 package, local state, Codex enablement hint를 점검합니다. |
...
| `codex-fable5 goals summary` | 최종 응답에 쓸 수 있는 progress, evidence, verification, findings-gate status를 출력합니다. |
...
marketplace source를 추가한 뒤에는 Codex Desktop의 plugin marketplace UI에서 `codex-fable5@fablecodex`를 설치하고 활성화하세요. plugin을 설치, 활성화, 업데이트한 뒤에는 Codex를 다시 시작하거나 plugin list를 reload하세요.

## 문제 해결

- marketplace를 추가했지만 `@codex-fable5`를 사용할 수 없다면: Codex Desktop에서 `codex-fable5@fablecodex`를 설치하고 활성화한 뒤 Codex를 다시 시작하거나 plugin list를 reload하세요.
- `codex plugin add`가 `error: unrecognized subcommand 'add'`를 출력한다면: 검증된 Codex runtime에서는 공개 CLI 명령이 아닙니다. `codex plugin marketplace add ...`를 사용한 뒤 Codex Desktop에서 plugin을 활성화하세요.
- 활성화 후에도 skill이 로드되지 않는다면: Codex를 다시 시작하고 프롬프트가 `@codex-fable5`를 쓰는지 확인하세요.
- `codex-fable5 update`가 dirty checkout 때문에 중단된다면: 업데이트 전에 local change를 commit, stash, 또는 clean하세요.
- provider bridge가 헷갈린다면: FableCodex는 workflow-only입니다. Fable, Anthropic, OpenAI, LiteLLM access를 제공하지 않으며, 선택적 bridge 설정에는 직접 보유한 valid credential과 authorized model access가 필요합니다.
